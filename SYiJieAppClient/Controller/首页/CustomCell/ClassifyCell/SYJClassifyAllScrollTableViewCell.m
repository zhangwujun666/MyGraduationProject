//
//  SYJClassifyAllScrollTableViewCell.m
//  SYiJieAppClient
//
...
@implementation SYJClassifyAllScrollTableViewCell{
    NSArray * ClassifyScrollImagesArray;
    NSTimer *timer;
}
...
-(void)getScrollImage{
    ...
    NSDictionary *info=@{@"homescrollimage_type":self.type
                         };
    ...
    [manager POST:urlPath parameters:info success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        ClassifyScrollImagesArray = dicdescription[@"data"];
        NSLog(@"dic=%@",ClassifyScrollImagesArray);
        [self setScrollImage];
...
- (void) setScrollImage{
    ...
    CGFloat imageY = 0;
    //添加图到ScorllView中
    for (int i=0; i<[ClassifyScrollImagesArray count]; i++) {
        // 设置frame
        ...
        //获取图片相对路径
        NSDictionary * dic = [ClassifyScrollImagesArray objectAtIndex:i];
        //获取图片路径
    ...
    // 2.设置内容尺寸
    CGFloat contentW = [ClassifyScrollImagesArray count] * imageW;
    ...
    // 5.设置pageControl的总页数
    self.pageContrl.numberOfPages = [ClassifyScrollImagesArray count];
...
-(void) nextImage{
    NSInteger page = 0;
    if (self.pageContrl.currentPage == [ClassifyScrollImagesArray count]-1) {
        page = 0;
    }
    else{
        page = self.pageContrl.currentPage +1;
    }
    // 2.计算scrollView滚动的位置
