...
@implementation SYJMiaoShaHeadView{
    NSString * Y;
    NSString * M;
    NSString * D;
    NSString * h;
...
- (void)setInit{
    ...
    if ([nowH intValue]<10 && [nowH intValue]>=0) {
        nowH = [NSString stringWithFormat:@"0%d",0];
    }
    ...
    if ([nowH isEqualToString:self.date]) {
        self.timeLabel.text = [NSString stringWithFormat:@"抢购中%@：00开抢",nowH];
        ...
        self.timeLabel.text = [NSString stringWithFormat:@"秒杀预告%@：00开抢",self.date];
        if ([self.date isEqualToString:@"0"]) {
            D = [NSString stringWithFormat:@"%d",[D intValue]+1];
            h = [NSString stringWithFormat:@"%d",0];
        ...
        if ([self.date isEqualToString:@"10"]) {
            if ([nowH intValue]>[self.date intValue]) {
                D = [NSString stringWithFormat:@"%d",[D intValue]+1];
            }
            h = [NSString stringWithFormat:@"%d",10];
        }
...
- (void)timerFireMethod:(NSTimer*)theTimer
{
    ...
    self.timeRemainingLabel.text = [NSString stringWithFormat:@"剩余%02d：%02d：%02d",[d hour], [d minute], [d second]];
